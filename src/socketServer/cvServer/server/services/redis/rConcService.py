...
class RedisForDetails:
    ...
    def clear(self, uid):
        ...
        :return:
        """
        key_1= self.__USEFUL_DETAILS_PREFIX + ':' + uid
        key_2= self.__DETAILS_PREFIX + ':' + uid
...
class RedisForConc:
    """ 封装有关专注度记录的数据库操作
        ...
        每位用户的记录在redis中以list形式存储
        一堂课的所有详细记录都将会存储在这个list中
        仅通过一个key（format: STATUS:CONC:LESSON:xxx）即可以访问到该课的所有记录
        或通过一个key（format: STATUS:CONC:UID:xxx）即可以访问到该用户所有的记录
    """
    __PRIMARY_KEY = 'LESSON_ID'
    __PRIMARY_KEY_UID = 'UID'

    __CONC = 'CONC'

    __PREFIX = REDIS_DB_NAME + ':' + __CONC + ':' + __PRIMARY_KEY
    __PREFIX_UID = REDIS_DB_NAME + ':' + __CONC + ':' + __PRIMARY_KEY_UID
    ...
    def addConcRecord(self, uid, course_id, lesson_id, begin_timestamp,
        ...
        # 需先data转换为str
        self.__conn.lpush(key, json.dumps(record_data))
        # format: STATUS:CONC:UID:xxx
        key = self.__PREFIX_UID + ':' + uid
        self.__conn.lpush(key, json.dumps(record_data))
    ...
    def getLastConcRecordByUid(self, uid):
        """ 获取该用户的最新一条专注度信息
        ...
        return json.loads(self.__conn.lpop(key))
